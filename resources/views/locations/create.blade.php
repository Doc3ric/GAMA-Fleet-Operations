<x-app-layout>
    ...
    @section('breadcrumb')
        <a href="{{ route('locations.index') }}" class="hover:underline">Location Directory</a> &bull; Add Location
    @endsection
    ...
    <div class="max-w-7xl mx-auto space-y-6" x-data="locationForm({
        initialLat: '{{ old('latitude', '14.599512') }}',
        initialLng: '{{ old('longitude', '120.984222') }}',
        initialAliases: {{ json_encode(old('aliases', [])) }}
    })">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-lg font-bold text-slate-900">Add Location Directory Record</h2>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('locations.store') }}" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            @csrf
            ...
            <div class="lg:col-span-7 space-y-5">
                ...
                <div class="rounded-2xl bg-white p-5 shadow-xs border border-slate-200 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="code" value="{{ old('code') }}" required placeholder="e.g. G2, CWH, GHO"
                    ...
                    <div>
                        ...
                        <input type="text" name="official_name" value="{{ old('official_name') }}" required placeholder="e.g. GAMA FARM 2, CENTRAL WAREHOUSE"
                        ...
                        <p class="text-[10px] text-slate-400 mt-1">Full authoritative official title used across operations and reports</p>
                    </div>
                </div>
                ...
                <div class="rounded-2xl bg-white p-5 shadow-xs border border-slate-200 space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500">Add informal names, abbreviations, or common slang drivers write in their itineraries</p>
                        </div>
                    ...
                    <div class="flex flex-wrap gap-1.5 min-h-[36px] p-2.5 rounded-xl border border-dashed border-slate-200 bg-slate-50/50">
                        ...
                        <template x-if="aliases.length === 0">
                            <span class="text-slate-400 text-xs italic self-center">No aliases added yet. Type an informal name and click Add.</span>
                        </template>
                    </div>
                </div>
                ...
                <div class="rounded-2xl bg-white p-5 shadow-xs border border-slate-200 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="2" required placeholder="Street address, building, km marker, industrial park..."
                                  class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">{{ old('address') }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-600 mb-1">Barangay</label>
                            <input type="text" name="barangay" value="{{ old('barangay') }}" placeholder="e.g. San Juan"
                        ...
                        <div>
                            ...
                            <input type="text" name="municipality" value="{{ old('municipality') }}" placeholder="e.g. Malolos City"
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">
                        </div>
                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-600 mb-1">Province</label>
                            <input type="text" name="province" value="{{ old('province') }}" placeholder="e.g. Bulacan"
                                   class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">
                        </div>
                    </div>
                ...
                <div class="rounded-2xl bg-white p-5 shadow-xs border border-slate-200 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 mt-1">Upload entrance, signage, or facility photo (max 5MB)</p>
                            ...
                            <template x-if="imagePreview">
                                <div class="mt-2.5 relative rounded-xl overflow-hidden border border-slate-200 max-h-40">
                                    ...
                                    <button type="button" @click="imagePreview = null" class="absolute top-1 right-1 rounded-full bg-slate-900/70 text-white h-5 w-5 flex items-center justify-center text-xs">&times;</button>
                                </div>
                            </template>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="4" placeholder="Access guidelines, landmarks, gate hours, contact personnel..."
                                      class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800 focus:bg-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
