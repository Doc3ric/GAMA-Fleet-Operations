    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                @can('delete', $location)
                    @php $linkedTrips = $location->trips()->count(); @endphp
                    <form method="POST" action="{{ route('locations.destroy', $location) }}"
                          onsubmit="return confirm('{{ $linkedTrips > 0 ? "This location is linked to {$linkedTrips} historical trips. It will be marked INACTIVE to protect history. Proceed?" : "Are you sure you want to permanently delete {$location->official_name}?" }}');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                class="inline-flex items-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-3.5 py-2 text-xs font-bold text-rose-700 hover:bg-rose-100 transition-colors shadow-xs cursor-pointer">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
